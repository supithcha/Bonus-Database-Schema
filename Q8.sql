...
-- ============================================================================
-- 4. LEGACY-COMPATIBLE VARIANTS (no window functions)
-- ============================================================================

-- ----------------------------------------------------------------------------
-- A5 (legacy). Employee with the highest (salary + bonus) in each department
-- ----------------------------------------------------------------------------
SELECT
    et.DEPARTMENT,
    et.FIRST_NAME,
...
WHERE et.TOTAL_COMP = (
    SELECT MAX(et2.TOTAL_COMP)
    FROM (
        SELECT
            w2.WORKER_ID,
            w2.DEPARTMENT,
            w2.SALARY + COALESCE(SUM(b2.BONUS_AMOUNT), 0) AS TOTAL_COMP
        FROM Worker w2
        LEFT JOIN Bonus b2 ON b2.WORKER_REF_ID = w2.WORKER_ID
        GROUP BY w2.WORKER_ID, w2.DEPARTMENT, w2.SALARY
    ) et2
    WHERE et2.DEPARTMENT = et.DEPARTMENT
)
ORDER BY et.DEPARTMENT;


-- ----------------------------------------------------------------------------
-- A6 (legacy). Title with the highest combined salary in each department
-- ----------------------------------------------------------------------------
SELECT
    tt.DEPARTMENT,
    tt.TITLE,
...
FROM (
    ...
    FROM Worker w
    LEFT JOIN Title t ON t.WORKER_REF_ID = w.WORKER_ID
    GROUP BY w.DEPARTMENT, COALESCE(t.WORKER_TITLE, 'Executive')
) tt
WHERE tt.TOTAL_SALARY = (
    SELECT MAX(tt2.TOTAL_SALARY)
    FROM (
        SELECT
            w2.DEPARTMENT,
            COALESCE(t2.WORKER_TITLE, 'Executive') AS TITLE,
            SUM(w2.SALARY) AS TOTAL_SALARY
        FROM Worker w2
        LEFT JOIN Title t2 ON t2.WORKER_REF_ID = w2.WORKER_ID
        GROUP BY w2.DEPARTMENT, COALESCE(t2.WORKER_TITLE, 'Executive')
    ) tt2
    WHERE tt2.DEPARTMENT = tt.DEPARTMENT
)
